    ...
    try:
        if proxy:
            async with create_async_http_client(
                ...
                body_limit = 2 * 1024 * 1024 if is_pdf else 4096
        ...
        if status != 200:
            logger.warning("zssm_explain: fetch group file failed, status=%s", status)
        else:
